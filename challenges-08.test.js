...
const getStatName = (input, minBaseStat) => {
  let answer = [];
  let arr = input.filter( obj => obj.baseStat > minBaseStat);
  for (let i = 0; i < arr.length; i++) {
    answer.push(arr[i].stat.name);
  }
  return answer;
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  let containsEvenOrOdd = [];
  arr.filter(element =>
  {
    if (Number.isInteger(element))
      (element % 2 === 0) ? containsEvenOrOdd.push('even') : containsEvenOrOdd.push('odd');
  });
  return containsEvenOrOdd;
};
