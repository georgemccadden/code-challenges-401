...
const listFoods = (recipe) => {
  ...
  for(let element of result) {
    newResults.push(element.slice(2));
  }

  for(let element of newResults) {
    finalResults.push(element.join(' '));
  }
...
const stepActions = (recipe) => {
  ...
  for (let i = 0; i < recipe.steps.length; i++){
    justTheVerbs.push(recipe.steps[i].split(' '));
  }
  for (let i = 0; i < justTheVerbs.length; i++){
    result.push(justTheVerbs[i][0]);
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if (numberOfCharacters < 0) {
    ...
    firstArray.splice(-numberOfCharacters);
    let resultingString = firstArray.join().replace(/,/g, '');
...
const totalSumCSV = (str) => {
  ...
  commaSeparated.forEach(element => {
