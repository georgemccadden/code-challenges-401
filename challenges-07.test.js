...
const evenOdd = (input) => {
  let resultingArray = input.map(value => {
    if(value % 2 === 0) {
      return 'even';
    }
    if(isNaN(value)) {
...
const extractStats = (stats) => {
  let resultingArray = stats.map(obj => {
    let newlyMadeObject = {};
    newlyMadeObject['name'] = obj.stat.name;
    newlyMadeObject['total'] = obj.effort + obj.baseStat;
    return newlyMadeObject;
  });
  return resultingArray;
};
